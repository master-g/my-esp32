...
{
    return (source == POWER_SOURCE_BATTERY) ? POWER_RUNTIME_BATTERY_DIM_TIMEOUT_US : POWER_RUNTIME_USB_DIM_TIMEOUT_US;
}

static int64_t get_sleep_timeout_us(power_source_t source)
{
    return (source == POWER_SOURCE_BATTERY) ? POWER_RUNTIME_BATTERY_SLEEP_TIMEOUT_US : POWER_RUNTIME_USB_SLEEP_TIMEOUT_US;
}
...
static void maybe_update_display_state_from_idle(void)
{
    ...
    if (input->display_state == DISPLAY_STATE_ACTIVE && idle_us >= get_dim_timeout_us(input->power_source)) {
        ESP_LOGI(TAG, "Idle timeout -> DIM");
        system_state_set_display_state(DISPLAY_STATE_DIM);
    } else if (input->display_state == DISPLAY_STATE_DIM && idle_us >= get_sleep_timeout_us(input->power_source)) {
...
esp_err_t power_runtime_init(void)
{
    ...
    ESP_RETURN_ON_FALSE(s_cmd_queue != NULL, ESP_ERR_NO_MEM, TAG, "failed to create power runtime queue");

    s_seen_activity_seq = system_state_get_user_activity_seq();
    s_last_activity_us = esp_timer_get_time();

    ESP_RETURN_ON_ERROR(event_bus_subscribe(power_runtime_event_handler, NULL), TAG, "failed to subscribe power runtime");

    xTaskCreatePinnedToCore(power_runtime_task,
                            "power_runtime",
                            POWER_RUNTIME_TASK_STACK,
                            NULL,
                            POWER_RUNTIME_TASK_PRIO,
                            &s_task_handle,
                            1);
