...
esp_err_t bsp_rtc_init(void)
{
    ...
    if (err == ESP_ERR_INVALID_STATE) {
        ESP_RETURN_ON_ERROR(i2c_master_get_bus_handle(BSP_SENSOR_I2C_PORT, &s_sensor_bus), "bsp_rtc", "sensor bus handle failed");
    } else if (err != ESP_OK) {
        return err;
    }

    {
        ...
        ESP_RETURN_ON_ERROR(i2c_master_bus_add_device(s_sensor_bus, &dev_cfg, &s_rtc_dev), "bsp_rtc", "rtc add device failed");
    }

    ESP_RETURN_ON_ERROR(read_registers(PCF85063_REG_CTRL1, &ctrl1, 1), "bsp_rtc", "rtc ctrl read failed");
    if (ctrl1 & PCF85063_REG_STOP_MASK) {
        ctrl1 &= (uint8_t)~PCF85063_REG_STOP_MASK;
        ESP_RETURN_ON_ERROR(write_registers(PCF85063_REG_CTRL1, &ctrl1, 1), "bsp_rtc", "rtc ctrl write failed");
    }
...
esp_err_t bsp_rtc_read_epoch(uint32_t *epoch_s)
{
    ...
    ESP_RETURN_ON_ERROR(read_registers(PCF85063_REG_SECONDS, regs, sizeof(regs)), "bsp_rtc", "rtc read failed");
    ESP_RETURN_ON_FALSE((regs[0] & PCF85063_REG_SECONDS_VALID_MASK) == 0,
                        ESP_ERR_INVALID_RESPONSE,
                        "bsp_rtc",
                        "rtc clock integrity not guaranteed");
    ...
    ESP_RETURN_ON_FALSE(local_time > 0, ESP_ERR_INVALID_RESPONSE, "bsp_rtc", "rtc returned invalid local time");
    ESP_RETURN_ON_FALSE(bsp_rtc_is_time_plausible((uint32_t)local_time),
                        ESP_ERR_INVALID_RESPONSE,
                        "bsp_rtc",
                        "rtc time not plausible");
