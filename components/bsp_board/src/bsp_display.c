...
static void lvgl_flush_cb(lv_display_t *display, const lv_area_t *area, uint8_t *color_p)
{
    ...
    if (rotation != LV_DISPLAY_ROTATION_0) {
        lv_draw_sw_rotate(color_p, s_rot_buf, src_w, src_h, src_stride, dest_stride, rotation, color_format);
    ...
    while (remaining_rows > 0) {
        ...
        esp_lcd_panel_draw_bitmap((esp_lcd_panel_handle_t)lv_display_get_user_data(display),
                                  0,
                                  row_offset,
                                  BSP_LCD_PANEL_H_RES,
                                  row_offset + rows,
...
static esp_err_t init_panel(void)
{
    ...
    ESP_RETURN_ON_ERROR(spi_bus_initialize(BSP_LCD_HOST, &buscfg, SPI_DMA_CH_AUTO), TAG, "lcd spi init failed");
    ESP_RETURN_ON_ERROR(esp_lcd_new_panel_io_spi(BSP_LCD_HOST, &io_config, &panel_io), TAG, "panel io init failed");
    ESP_RETURN_ON_ERROR(esp_lcd_new_panel_axs15231b(panel_io, &panel_config, &s_panel), TAG, "panel driver init failed");
...
static esp_err_t init_lvgl(void)
{
    ...
    ESP_RETURN_ON_FALSE(s_lvgl_mutex != NULL && s_flush_done_semaphore != NULL,
                        ESP_ERR_NO_MEM,
                        TAG,
    ...
    ESP_RETURN_ON_FALSE(s_psram_buf_1 != NULL && s_psram_buf_2 != NULL && s_rot_buf != NULL && s_dma_buf != NULL,
                        ESP_ERR_NO_MEM,
                        TAG,
                        "display buffers alloc failed");
    ...
    ESP_RETURN_ON_ERROR(esp_timer_create(&lvgl_tick_timer_args, &lvgl_tick_timer), TAG, "tick timer create failed");
    ESP_RETURN_ON_ERROR(esp_timer_start_periodic(lvgl_tick_timer, BSP_LVGL_TICK_MS * 1000), TAG, "tick timer start failed");

    xTaskCreatePinnedToCore(lvgl_port_task,
                            "bsp_lvgl",
                            BSP_LVGL_TASK_STACK_SIZE,
                            NULL,
...
{
    bsp_backlight_set_percent(percent);
}
