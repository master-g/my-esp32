...
static void set_placeholder(void)
{
    ...
    snprintf(s_snapshot.hhmm, sizeof(s_snapshot.hhmm), "%s", "--:--");
...
static esp_err_t load_timezone_from_nvs(void)
{
    ...
    if (err == ESP_ERR_NVS_NOT_FOUND || s_timezone[0] == '\0') {
        ...
        ESP_RETURN_ON_ERROR(nvs_set_str(handle, "timezone", s_timezone), TAG, "failed to seed timezone");
        ESP_RETURN_ON_ERROR(nvs_commit(handle), TAG, "failed to commit timezone");
        err = ESP_OK;
    }
    nvs_close(handle);
...
static void update_text_from_epoch(uint32_t epoch_s)
{
    ...
    snprintf(s_snapshot.hhmm, sizeof(s_snapshot.hhmm), "%02d:%02d", tm_value.tm_hour, tm_value.tm_min);
    snprintf(s_snapshot.date_text,
             sizeof(s_snapshot.date_text),
             "%02d/%02d",
             tm_value.tm_mon + 1,
             tm_value.tm_mday);
    snprintf(s_snapshot.weekday_text,
             sizeof(s_snapshot.weekday_text),
             "%s",
             s_weekdays[(tm_value.tm_wday >= 0 && tm_value.tm_wday < 7) ? tm_value.tm_wday : 0]);
}
...
static esp_err_t do_sntp_sync(void)
{
    ...
    while ((err = esp_netif_sntp_sync_wait(pdMS_TO_TICKS(2000))) == ESP_ERR_TIMEOUT && ++retry < retry_count) {
...
static void time_service_event_handler(const app_event_t *event, void *context)
{
    ...
    if (event->type == APP_EVENT_NET_CHANGED && net_manager_is_connected() && !s_ntp_synced && !s_sync_in_progress) {
...
esp_err_t time_service_init(void)
{
    ...
    ESP_RETURN_ON_ERROR(event_bus_subscribe(time_service_event_handler, NULL), TAG, "failed to subscribe to event bus");
...
esp_err_t time_service_start(void)
{
    ...
    ESP_RETURN_ON_FALSE(s_command_queue != NULL, ESP_ERR_NO_MEM, TAG, "failed to create time queue");
...
esp_err_t time_service_sync_ntp(void)
{
    ...
    ESP_RETURN_ON_FALSE(net_manager_is_connected(), ESP_ERR_INVALID_STATE, TAG, "network not connected");
    if (s_sync_in_progress) {
        return ESP_OK;
    }
...
{
    return s_rtc_valid || s_ntp_synced;
}
