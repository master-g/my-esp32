...
{
    ESP_RETURN_ON_FALSE(s_initialized, ESP_ERR_INVALID_STATE, "event_bus", "event bus not initialized");
    ESP_RETURN_ON_FALSE(handler != NULL, ESP_ERR_INVALID_ARG, "event_bus", "handler is required");
    ESP_RETURN_ON_FALSE(s_subscription_count < EVENT_BUS_MAX_SUBSCRIBERS,
                        ESP_ERR_NO_MEM,
                        "event_bus",
