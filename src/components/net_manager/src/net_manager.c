...
static char s_wifi_password[NET_MANAGER_PASSWORD_MAX];
...
static esp_err_t apply_wifi_config(void)
{
    ...
    memcpy(wifi_config.sta.ssid, s_snapshot.ssid,
           bounded_cstr_len(s_snapshot.ssid, sizeof(wifi_config.sta.ssid)));
    memcpy(wifi_config.sta.password, s_wifi_password,
...
static esp_err_t seed_credentials_from_defaults(nvs_handle_t handle)
{
    ...
    if (!has_text(s_snapshot.ssid) && has_text(CONFIG_DASH_WIFI_SSID)) {
        ESP_RETURN_ON_ERROR(nvs_set_str(handle, NET_NVS_KEY_SSID, CONFIG_DASH_WIFI_SSID), TAG,
                            "failed to store default ssid");
        changed = true;
    }
    ...
    if (!has_text(s_wifi_password) && has_text(CONFIG_DASH_WIFI_PASSWORD)) {
        ESP_RETURN_ON_ERROR(nvs_set_str(handle, NET_NVS_KEY_PASSWORD, CONFIG_DASH_WIFI_PASSWORD),
                            TAG, "failed to store default password");
        changed = true;
    }
...
static esp_err_t load_credentials_from_nvs(void)
{
    ...
    if (err == ESP_OK) {
        err =
            read_nvs_string(handle, NET_NVS_KEY_PASSWORD, s_wifi_password, sizeof(s_wifi_password));
    }
    if (err == ESP_OK) {
...
static void schedule_retry(void)
{
    ...
    delay_ticks = pdMS_TO_TICKS((s_disconnect_count < NET_FAST_RETRY_COUNT) ? NET_FAST_RETRY_MS
                                                                            : NET_SLOW_RETRY_MS);
    s_disconnect_count++;
...
esp_err_t net_manager_start(void)
{
    ...
    ESP_RETURN_ON_ERROR(apply_wifi_config(), TAG, "failed to apply Wi-Fi config");
...
void net_manager_get_credentials_summary(net_credentials_summary_t *summary)
{
    ...
    snprintf(summary->ssid, sizeof(summary->ssid), "%s", s_snapshot.ssid);
    summary->has_password = has_text(s_wifi_password);
}

esp_err_t net_manager_apply_credentials(const char *ssid, const char *password)
{
    if (ssid == NULL || strlen(ssid) >= sizeof(s_snapshot.ssid)) {
        return ESP_ERR_INVALID_ARG;
    }
    if (password == NULL || strlen(password) >= sizeof(s_wifi_password)) {
        return ESP_ERR_INVALID_ARG;
    }
                                         ...
                                         size_t *out_count)
{
    ...
    *out_count = 0;
    ...
    if (s_started) {
        ESP_RETURN_ON_ERROR(apply_wifi_config(), TAG, "failed to refresh Wi-Fi config before scan");
    }
    ...
    for (i = 0; i < ap_count && *out_count < max_results; ++i) {
        ...
        snprintf(results[*out_count].ssid, sizeof(results[*out_count].ssid), "%s",
