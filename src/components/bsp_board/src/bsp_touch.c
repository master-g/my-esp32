...
esp_err_t bsp_touch_init(void)
{
    ...
    ESP_RETURN_ON_ERROR(i2c_new_master_bus(&bus_config, &s_touch_bus), "bsp_touch",
                        "touch bus init failed");
    ESP_RETURN_ON_ERROR(i2c_master_bus_add_device(s_touch_bus, &dev_cfg, &s_touch_dev), "bsp_touch",
                        "touch dev add failed");
    s_touch_ready = true;
    return ESP_OK;
}
...
esp_err_t bsp_touch_read(bsp_touch_point_t *point)
{
    ...
    ESP_RETURN_ON_ERROR(i2c_master_transmit_receive(s_touch_dev, cmd, sizeof(cmd), buf, sizeof(buf),
                                                    pdMS_TO_TICKS(100)),
                        "bsp_touch", "touch read failed");
