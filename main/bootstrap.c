...
static esp_err_t register_apps(void)
{
    ...
    for (i = 0; i < sizeof(descriptors) / sizeof(descriptors[0]); ++i) {
        ESP_RETURN_ON_ERROR(app_manager_register(descriptors[i]), TAG, "failed to register app %u", (unsigned)i);
    }
...
esp_err_t bootstrap_start(void)
{
    ...
    ESP_RETURN_ON_ERROR(event_bus_subscribe(app_manager_on_event, NULL), TAG, "failed to subscribe app manager");
...

    {
        ...
        ESP_RETURN_ON_ERROR(esp_timer_start_periodic(s_tick_timer, 1000000), TAG, "tick timer start failed");
    }
