...
- 当前 `make build` 结果：
  - app binary size: `0x335590`
  - smallest app partition free: `0x0caa70`（约 20%）
...
不要再走 LVGL transform rotation。这块板子的现有渲染链路是 full render + software rotation，object transform 的代价和风险都太高。可行方向只剩两类：

1. 直接做 **pre-baked sway assets**，把倾斜感烘进 sprite frame；
2. 继续沿用 bob 的位置系统，只尝试很轻的 **x-translate / asymmetric bob**，不要碰 `transform_rotation`。

**当前验证结果**

在回退到 bob-only 后，以下链路已重新通过设备 smoke：
...
2. 只有在找到**不依赖 transform_rotation** 的替代方案后，才值得继续试 tremble / sway
