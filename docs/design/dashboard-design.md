...
- 时间周期：`5M`、`1H`、`4H`、`1D`
- 默认周期由 `Settings > Crypto Market > Chart Interval` 管理，默认值为 `5M`
- price digits 可选切到 Binance 风格的绿涨红跌；关闭时保持主题默认颜色
- 图表刷新节流到最多 `1 FPS`
...
- 审批请求会自动切回 `Home`，并以全屏 overlay 呈现
...
- 时间周期切换和 price digits 颜色风格切换移到 `Settings > Crypto Market`
- 不使用内容区左右滑动切换交易对
