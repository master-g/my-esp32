...
当前实现先走 `REST polling`，但 `market_feed` 抽象从一开始就保留到 `WebSocket` 的切换位。
同时采用双源策略：
...
| 子模块 | 职责 |
|------|------|
| `market_feed` | 管理上游采集；当前为 REST，后续可替换为 WebSocket |
...
- 前台 `Trading` 页：优先实时更新
- 当前实现先用秒级 REST 轮询
- 后续可以把价格摘要切到 WebSocket，但不改页面层接口
